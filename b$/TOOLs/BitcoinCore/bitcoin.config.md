# Die Konfigurationsdatei für den lokal installierten Blockchain-Knoten

* [bitcon.conf-Datei-Beschreibung auf GitHub](https://github.com/bitcoin/bitcoin/blob/master/doc/bitcoin-conf.md)

* [Dokumentiertes, GUI-Tool zum dynamischen Erstellen von Bitcoin.conf-Dateien]( https://jlopp.github.io/bitcoin-core-config-generator/)

---

All command-line options (except for -?, -help, -version and -conf) may be specified in a configuration file, and all configuration file options (except for includeconf) may also be specified on the command line. 
...
## Zusammenfassung
Die ***bitcoin.conf* Konfigurationsdatei** wird von allen drei Bitcoin-Core Komponenten verwendet: 
1. vom BitCoin **Daemon** (*bitcoind*) 
2. vom BitCoin Core **KommandozeilenTool** (*bitcoin-cli*) 
3. dem Bitcoin **Core GUI** (*bitcoin-qt*)

* **bitcoin.config Location**: **D:\@ME\DATA\BitcoinBlockchainData** (dies ist mein eigene, aktuelle location die anders von der in der Doku beschriebenen Default Location auf  C:\Users\RealB12\AppData\Local\Bitcoin) abweicht. 

* **RPC Port** für RPC-Calls: 8332

* Die aktuell Location für die KonfigDatei erhält man indem man im GUI auf "**Settings**" -> "**Options**" und dort auf dem  Button **[Open Configuratoin File]** klickt. 
...
* **S:** [server (RPC API)](#server-rpc-api)

* **T:** [txindex](#txindex)


### regtest (Chain Selection)
Mit 
...
erstellt der Daemon nach dem Neustart eine eigene, lokale, private und damit zu 100% von der richtigen Blockchain (main) und Test entkoppelte Regressions-Test-Netzwerk (resp. eine eigene, lokale Blockchain), wie sie z.B. für die Entwicklung und das Testen eigener Bitcoin-Tools und Wallets benötigt werden. Damit kann man natürlich keine echten Transaktionen starten oder die aktuellen Werte der echten Chain einsehen. Dafür kann man z.b.  innert Sekunden eigene Blocks erstellen und diesbezügliche Features testen.  

**What it does**: 
1. Creates a private network: It sets up a local, separate blockchain environment, isolated from mainnet (real Bitcoin) and testnet (public test network).

2. Enables rapid block creation: You can instantly generate blocks (e.g., by using *setgenerate true 1* or *setgenerate true 101*), allowing for immediate transaction confirmations and testing time-sensitive features like timelocks.

3. Simulates specific conditions: Developers can set up exact network parameters to test specific features or edge cases that might be hard to reproduce on public networks. 

### server (RPC API)
tells Bitcoin-Qt to accept JSON-RPC commands? 
...
Mit aktivierten ServerZugang sollte man dann auch wie folgt mit den beiden folgenden Kommandos Credentials für Servercalls erstellen: 

    rpcuser=SOMEUSER
    rpcpassword=SOMEPASSWORD
    
if you set server=1 in your bitcoin.conf file your wallet will run a server that listens for and accepts commands passed to it. These can be used to externally control the wallet. By default only requests from your local machine are accepted. JSON is the formatting language the commands are issued in. JSON_RPC is a standardized way of issuing these commands using the json format. When you run the client you can use the Console available from the help menu to run the same set of commands directly.

### txindex
By default, Bitcoin Core builds a database containing only the transactions related to the user’s wallet. If you want to be able to access any transaction with commands like *gettransaction* or you are running smart contracts that needs to access data beyond your own reach, you need to configure Bitcoin Core to build a complete transaction index, which can be achieved with the *txindex* option. Set *txindex=1* in the Bitcoin Core configuration file. The tradeoff is just that keeping an index is slightly slower

Also, if you have been running your client for a while but haven't had txindex=1 set, then it might take a few hours to build the index. If you want to rebuild, launch with the -reindex option (however, this doesn't re-download the data, because you should already have the data on disk, it just reprocesses the data you have). 

---

## Generelles Config-Datei-Handling

### Configuration File Path
The configuration file **is not automatically created**. 

By default, the configuration file name is *bitcoin.conf* and it is located in the user's "C:\Users\RealB12\AppData\Local\Bitcoin" AppData directory. 

However, the Bitcoin data directory and the configuration **file path may be changed using the *-datadir* and *-conf* command-line options**.
...
#### Unterschiede zu Kommandozeilen Optionen
...
1. on their own on an otherwise empty line (preferable)
2. after an option=value entry.

#### Network spezifische Optionen
Network specific options can be:

1. placed into sections with headers [main] (not [mainnet]), [test] (not [testnet], for testnet3), [testnet4], [signet] or [regtest];

2. prefixed with a chain name; e.g., regtest.maxmempool=100.
...
## Default Config Beispiel
Die original bitcoin.conf-Datei findet sich bei mir im **C:\me\TOOLS\BitcoinCore\bitcoin-28.1** Installationsordner. 

<span style="color:red; font-weight:bold">ACHTUNG</span>:Das ist nur eine Sicherheitskopie. Die WIRKSAME Datei findet sich im DatenVerzeichnis: 
